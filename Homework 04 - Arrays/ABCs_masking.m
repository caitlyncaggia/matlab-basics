...
% full array, and get an array of logical data. This is also called a Mask,
% and the act of using a mask to manipulate an array is called Masking.
% Create a mask of where arr1 is equal to 0. (A)
   A = arr1==0;

% 2. Indexing with a mask: Data is manipulated with masks by using Logical
% Indexing, which is where one uses a mask to index an array of the same
% size (and usually the array that was used to create the mask in the first
% place). Use the mask you created in part A to assign a 1 into all the
% indices of arr1 that are 0. Assign the new arr1 to output B. (B)
    arr1(A) = 1
    B = arr1
 
% 3. The mod() function may also be used with arrays. Use mod() to create a
% mask of where arr1 contains an even number. (C)
    C = mod(arr1,2)==0;
...
% even values in arr1 into the corresponding values of arr2. Then assign
...
% this to find a mask of where both A and C were true. (Note: && and ||
